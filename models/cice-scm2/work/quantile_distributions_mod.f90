...
real(r8) :: bound_inv, correction, difference
...
!------------------- End unused block -------------------------------
   ...
   ! -- BLOCK TO ENSURE THAT THERE ARE NO VIOLATIONS OF THE UPPER BOUND-- adapted by MW from JLA ---------!
      else !if(bounded_above) then
         ! Find mass at the upper bound
         mass = tail_amp_right * norm_cdf(upper_bound, tail_mean_right, tail_sd_right)
         ! If the inverse for the boundary gives something less than the bound have to fix it
         call weighted_norm_inv(tail_amp_right, tail_mean_right, tail_sd_right, mass, bound_inv)
         difference = upper_bound - bound_inv
         correction = abs(min(0.0_r8, difference))
         target_mass = mass + quantile
         ! write(msg1, *) 'correction = ', correction, ' and probit ensemble member = ', state_ens(i)
         call weighted_norm_inv(tail_amp_right, tail_mean_right, tail_sd_right, target_mass, state_ens(i))
         ! write(msg2, *) 'inverse norm ensemble member = ', state_ens(i), ', tail_mean_right = ', tail_mean_right, ', tail_sd_right = ', tail_sd_right
         if (correction > 0.0_r8) then
            state_ens(i) = upper_bound
            ! write(msg3, *) 'corrected state member = ', state_ens(i)
            ! call error_handler(E_ERR,'quantile_distributions:', msg1, source, &
