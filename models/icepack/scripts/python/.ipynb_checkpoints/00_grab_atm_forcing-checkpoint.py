...
for p in range(len(LatsOfInterest)):
    ...
    for m in range(mems.shape[0]):
        ...
        t_10 = list(xr.open_mfdataset(t_10_files).t_10.values[:, i, j][:, 0])
        u_10 = list(xr.open_mfdataset(u_10_files).u_10.values[:, i, j][:, 0])
        v_10 = list(xr.open_mfdataset(v_10_files).v_10.values[:, i, j][:, 0])
        q_10 = list(xr.open_mfdataset(q_10_files).q_10.values[:, i, j][:, 0])
        swdn = list(xr.open_mfdataset(swdn_files).swdn.values[:, i, j][:, 0])
        lwdn = list(xr.open_mfdataset(lwdn_files).lwdn.values[:, i, j][:, 0])
        prec = list(xr.open_mfdataset(prec_files).prec.values[:, i, j][:, 0])
