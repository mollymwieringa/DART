...
!autodocument_end
      ...
      !-----------------------------------------------------------------
      ! Remove any small negative values from ice volume
      !-----------------------------------------------------------------
      ! NOTE: This is a temporary fix for the problem of negative ice
      !       volume implemented by Molly Wieringa

      do n = 1, ncat
         if (vicen(n) < c0) then
            if (abs(vicen(n)) < puny) then
               vicen(n) = c0
            else
               call icepack_warnings_add(subname//' negative ice volume < -puny')
               call icepack_warnings_setabort(.true.,__FILE__,__LINE__)
            endif
         elseif (vicen_init(n) < c0) then
            if (abs(vicen_init(n)) < puny) then
               vicen_init(n) = c0
            else
               call icepack_warnings_add(subname//' negative initial ice volume < -puny')
               call icepack_warnings_setabort(.true.,__FILE__,__LINE__)
            endif
         endif
      enddo
